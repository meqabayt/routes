 ...
 */
@Singleton
class HomeController @Inject()(graphQL: GraphQL, cc: ControllerComponents, implicit val executionContext: ExecutionContext) extends AbstractController(cc) {
  ...
  def graphiql() = Action { implicit request: Request[AnyContent] =>
    Ok(views.html.explorer())
  }
